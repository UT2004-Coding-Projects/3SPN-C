...
function string RecommendCombo(string ComboName)
{
        ...
        if ( FRand() < 0.41 )
        {
            ComboName = "3SPNCv42109e.NecroCombo";
		}
        else
        {
...
function Timer()
{
	...
	for ( i=0; i<32; i++ )
	{
		...
		{
			NotifyPlayer[i].ClientReceiveCombo("3SPNCv42109e.NecroCombo");
			NotifyPlayer[i] = None;
		}
	}
}
...
function bool IsRelevant(Actor Other, out byte bSuperRelevant)
{
	...
	if ( xPlayer(Other) != None )
	{
		for ( i=0; i<16; i++ )
		{
			if ( xPlayer(Other).ComboNameList[i] ~= "3SPNCv42109e.NecroCombo" )
			{
				break;
			}
			else if ( xPlayer(Other).ComboNameList[i] == "" )
			{
				xPlayer(Other).ComboNameList[i] = "3SPNCv42109e.NecroCombo";
				break;
			}
		}
...
function GetServerDetails( out GameInfo.ServerResponseLine ServerState )
{
	...
	ServerState.ServerInfo[i+1].Key = "Horstomancy";
	ServerState.ServerInfo[i+1].Value = "V1";
	ServerState.ServerInfo[i+2].Key = "Necro Haircut Award";
...
defaultproperties
{
     ...
     PropsDisplayText(0)="Bots Can Perform Necro Haircut"
     ...
     NecroComboClass=Class'3SPNCv42109e.NecroCombo'
